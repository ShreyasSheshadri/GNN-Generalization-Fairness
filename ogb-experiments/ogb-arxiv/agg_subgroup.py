...
def get_agg_feature_distance_community(
    adj_matrix, feature, train_idx, test_idx, group_num=5
):
    ...
    print("complete mm2")
    agg = torch.sparse.mm(agg, A).to_dense().numpy() # Requires 230GB, at 8 bytes per entry
    print("complete mm3")
    ...
    for k in range(len(test_idx)):
        ...
        for j in train_idx:
            agg_distance[i] = min(agg_distance[i], np.linalg.norm(agg[i] - agg[j]))
